...
public class TypeProductDAO {
	...
	public List<TypeProduct> select() {
        String sql = """
...
//        DatabaseHelper.executeUpdate(sql, model.getCodeProduct(), model.getCodeProductType(), model.getCodeSupplier(), model.getDOM(), model.getED());
    ...
    private List<TypeProduct> select(String sql, Object... args) {
        ...
        try {
            ...
            try {
                ...
                while (rs.next()) {
                    TypeProduct model = readFromResultSet(rs);
                    list.add(model);
                }
            } finally {
    ...
    private TypeProduct readFromResultSet(ResultSet rs) throws SQLException {
